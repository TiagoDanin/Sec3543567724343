...
export function BioHeader({ logo, handle, lede, facts, className }: BioHeaderProps) {
  return (
    <header className={cn("text-center", className)}>
      ...
      {facts?.length ? (
        <ul className="border-line text-cream-3 mt-5 flex flex-wrap justify-center gap-y-2 gap-x-[18px] border-t pt-[18px] font-mono text-[12px] uppercase tracking-[0.12em]">
...
export function SocialRow({ links, className, ...props }: SocialRowProps) {
  return (
    <nav
      className={cn(
        "border-line mt-[clamp(28px,5vw,40px)] flex flex-wrap justify-center gap-y-2.5 gap-x-[22px] border-t pt-[22px] font-mono text-[12px] uppercase tracking-[0.14em]",
        className,
      )}
      {...props}
    >
      {links.map((link) => (
        <a
          ...
          target="_blank"
          rel="noopener"
          className="text-cream-2 ease-brand border-b border-transparent transition-colors duration-250 hover:text-orange hover:border-orange focus-visible:text-orange focus-visible:border-orange"
        >
          {link.label}
        </a>
      ))}
